...
OFFLINE_DATASET="${BASE_GAP_ROOT}/dataset_expert_command_25k.pt"
...
CODEX_LOCK="${BASE_ROOT}/trace_method_v1/codex_feedback.lock"
...
if [[ ! -f "${POLICY_STAGE}/summary.json" ]]; then
  ...
  code=${PIPESTATUS[0]}
  if (( code != 0 )); then
    write_state train_final_policy_trace failed
    exit "${code}"
  fi
fi
