...
TRACE_ACTION_TEMPERATURE="${TRACE_ACTION_TEMPERATURE:-4.0}"
...
CANDIDATES="${RUN_ROOT}/imperfect_sim/candidates_t4_1024x4x20.pt"
...
if [[ ! -f "${CANDIDATES}" ]]; then
  run_stage collect_imperfect_sim_candidates "${PYTHON_BIN}" \
    ...
    --trace_rollout_length 20 \
    --trace_trajectories_per_state 4 \
...
train_policy_ratio() {
  ...
  if [[ -f "${policy_stage}/summary.json" ]]; then
    echo "[reuse] completed final policy ${ratio_id}" | tee -a "${RUN_LOG}"
    return
  fi
